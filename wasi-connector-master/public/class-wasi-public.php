...
class Wasi_Connector_Public {
  ...
  public function __construct($plugin_name, $version, $lang_context = 'wasico') {
    ...
    add_shortcode('wasi-properties', [$this, 'wasiPropertiesShortcode']);
    add_shortcode('wasi-single-property', [$this, 'wasiSingePropertyShortcode']);
    add_shortcode('wasi-search', [$this, 'initSearchShortcode']);

    add_action('widgets_init', [$this, 'initWasiWidgets']);

    add_filter('template_include', [$this, 'wasi_single_page'], 99);
  }
  ...
  public function wasi_single_page($template) {
    ...
    if(is_object($wp_wasi_post) && is_object($post) && $wp_wasi_post->ID === $post->ID) {
      ...
      if(is_numeric($id_property)) {
        ...
        add_filter('the_content', [$this, 'wasiSingleProperty'], 1);
        ...
        wp_enqueue_script('unitejs', $unite_js_url, ['jquery'], '1.7.28', true);
        wp_enqueue_style('unitecss', $unite_css_url, [], '1.7.28', 'all');
        ...
        wp_enqueue_script('unitejs-theme', $unite_js_url, ['unitejs'], '1.7.28', true);
        wp_enqueue_style('unitecss-theme', $unite_css_url, ['unitecss'], '1.7.28', 'all');
      }
    }
  ...
  public function initWasiWidgets() {
    ...
    register_sidebar([
                       'name'          => 'Wasi Sidebar',
                       'id'            => 'wasi-sidebar',
                       'description'   => 'Shows on the property detail page',
                       'class'         => '',
  ...
  public function enqueue_styles() {
    wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/wasi-connector-public.css', [], $this->version, 'all');
 wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/flexslider.css', [], $this->version, 'all');
  wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/flexslider-rtl.css', [], $this->version, 'all');
    ...
    if($use_bootstrap === 'true') {
      ...
      wp_enqueue_style('bootstrap', $bootstrap_css, [], null, 'all');
    }
  }
  ...
  public function enqueue_scripts() {
    ...
    wp_enqueue_script('vuejs', $vue_url, ['jquery'], '2.5.6', true);


    // wp_enqueue_script( 'moonjs', 'https://unpkg.com/moonjs', array( 'jquery' ), '0.11.0', true );
    wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/wasi-connector-public.js', ['vuejs'], $this->version, true);
wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/modernizr.js', ['vuejs'], $this->version, true);
    $use_bootstrap = isset($this->api_data['wasi_bootstrap']) ? $this->api_data['wasi_bootstrap'] : '';
    if($use_bootstrap === 'true') {
      $bootstrap_js = 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js';
      wp_enqueue_script('bootstrap', $bootstrap_js, ['jquery'], null, true);
    }

    // add data before JS plugin
    // $js_vars = ;
    add_action('wp_footer', [$this, 'createPublicJSvars'], 1);
