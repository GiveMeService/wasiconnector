...
<script type='text/javascript' src='/wp-content/plugins/wasi-connector-master/public/js/modernizr.js'></script>
<script type='text/javascript' src='/wp-content/plugins/wasi-connector-master/public/js/pgw-sliders/pgwslider.min.js'></script>

<script>var propertyTypes = [];</script>

<div id="wasiAppSingle" class="listing-detail pageProperties1">
    ...
    <?php if ($single_property->galleries) {
      ...
      foreach ($imgs as $key => $value) {
        if (is_object($value)) {
          ...
          $images_html.= "
              <li>
                <a href='{$value->url}' target='_self' class='vc_single_image-wrapper vc_box_border_grey prettyphoto'>
                  <img src='{$value->url}' "
                    ."data-image='{ $value->url_big }' "
                    ."data-large-src='{ $value->url_big }' "
                    ."data-description='{ $value->description }' "
                    ."alt='{$value->description}' /></a></li>";
        }
      }
      ...
      if(strlen($images_html) > 0) {
        echo '<div id="wasi_gallery_container">
                <div id="wasi_gallery" style="visibility:hidden;">
                  <ul class="slides" style="text-align: center;">'.$images_html.'</ul>
                </div>
    ...
    ?>
        <script>
          jQuery(document).ready(function($) {
              ...
              $('#wasi_gallery').flexslider({
                animation: "slide",
                controlsContainer: $(".custom-controls-container"),
                customDirectionNav: $(".custom-navigation a")
              });
              g.css('visibility', 'visible');
			        //jQuery('.custom-navigation p').contents().unwrap();
          });
        </script>
